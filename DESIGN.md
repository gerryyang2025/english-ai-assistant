...
- **浏览器兼容性：支持 Chrome 浏览器（桌面版和移动版）以及 iPad Chrome APP 模式**
- 利用 Chrome 专有特性优化用户体验（如高级 CSS 动画、Web Speech API 完整支持）
- **iPad 优化：完整的 iPad Chrome APP 模式支持，包括 viewport-fit=cover、安全区域适配、动态视口高度（dvh/svh）**
...
### 5.2 响应式设计（Chrome 浏览器优化，包含 iPad 支持）
采用移动端优先策略，五个断点适配不同设备（Chrome 浏览器特性优化）：
- 默认：桌面设备（>768px）
- iPad：768px - 1024px（iPad 专用优化）
- 平板：481px - 768px
- 手机：361px - 480px
- 小屏：≤360px
- **利用 Chrome 专有 CSS 特性实现流畅动画效果**
- **iPad 专用：动态视口高度、安全区域适配、底部控制栏防遮挡**
